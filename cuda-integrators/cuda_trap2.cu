 ...
 *           size.  It uses warp shuffle to add the values within a warp. 
 *           This version adds the warp sums using atomicAdd
 *           We assume that (blk_ct-1)*32 < n <= blk_ct*32.  This insures 
 *           that every block gets some work.
 *
 * Compile:  nvcc -arch=sm_XX -o trap2a trap2a.cu 
 *              XX >= 3.0
 * Run:      ./trap2a <n> <a> <b> <blk_ct>
 ...
 *
 * Runtimes 2/5/17 n = 2^20, a = -3, b = 3, blk_ct = 32768
...
__global__ void Dev_trap(
   ...
   /* result is correct only on for thread 0 */
   if (threadIdx.x == 0) {
#     ifdef DEBUG
...
#        endif
      ...
      /* Tree structured sum to lane 0 */
      for (unsigned bit_mask = WARPSZ >> 1; bit_mask > 0; 
            bit_mask >>= 1) {
         for (lane = 0; lane < bit_mask; lane++) {
            unsigned partner = lane | bit_mask;
            temp[lane] += temp[partner];
         }
      }  /* for bit_mask */
#     ifdef DEBUG
