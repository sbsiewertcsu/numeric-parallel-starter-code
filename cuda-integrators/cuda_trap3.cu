 ...
 *           version uses warp shuffle to add the values within a warp. 
 *           It then uses warp shuffle to add the warp sums in a block.
 *           It adds the block sums using atomicAdd.  We assume that
 *           (blk_ct-1)*th_per_blk < n <= blk_ct*th_per_blk.  This insures 
 *           that every block gets some work.  It also assumes that the
 *           maximum block size is 1024 and the block size is a multiple
 *           of the warp size.
 *
 * Compile:  nvcc -o trap3a trap3a.cu -arch=sm_32 
 *              arch must be >= 3.2
 * Run:      ./trap3a <n> <a> <b> <blk_ct> <th_per_blk>
...
__device__ float Warp_sum(const float my_val) {
   ...
   float my_result = my_val;

   for (unsigned diff = warpSize/2; diff > 0; diff = diff/2) {
      my_result += __shfl_down_sync(mask, my_result, diff);
   }
...
#  endif
   ...
   if (my_warp == 0) {
      ...
      my_trap = warp_sum_arr[threadIdx.x];
#     ifdef DEBUG
      printf("GPU:  th = %d, add = %e\n", my_i, my_trap);
#     endif
      blk_result = Warp_sum(my_trap);
#     ifdef DEBUG
