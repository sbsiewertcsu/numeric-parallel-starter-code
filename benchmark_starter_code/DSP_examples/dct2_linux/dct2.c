...
//
// After verifying that Emil's code works using the DCT and iDCT and Octave, 
// I then adapted this to be a basich benchmark for the DCT (used in MPEG for
// example).  However, note that this algorithm is not the known fastest, so
// it's mostly useful to learn about the DCT, the inverse and to get an idea
// how fast your system is for these types of transforms.  I would not
// use this code as a basis for a software encoder/decoder.  For that, see
// my reference on the AAN algorithm below.
//
...
int main()
{
    ...
    // This is called up to MAX_ITERATION times in a loop as a benchmark

    printf("Starting DCT test for %d iterations\n", MAX_ITERATION);
    for(idx=0; idx < MAX_ITERATION; idx++)
    {
        dct(Macroblock, dct2);
        idct(dct2, idct2);
    }
    printf("Completed DCT test for %d iterations\n", idx);
