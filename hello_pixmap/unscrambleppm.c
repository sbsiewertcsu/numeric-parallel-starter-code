...
static int descramble_image(PPMImage *img, uint64_t key) {
    ...
    restored = (Pixel *)malloc(n * sizeof(Pixel));
    if (!restored) {
        free(perm);
        fprintf(stderr, "Error: out of memory\n");
        return 0;
...
int main(int argc, char *argv[]) {
    ...
    if (argc != 4) {
        fprintf(stderr, "Usage: %s <scrambled.ppm> <output.ppm> <key>\n", argv[0]);
        return 1;
    }
    ...
    free_ppm(&img);
    printf("Descrambled image written to %s\n", argv[2]);
    return 0;
}
